...
#if 1 /* Renesas EIP Sample : CT18.1 Error "Did not receive T->O I/O packets *** ms after packet with Sequence Number 100! Time since last HeartBeat *** ms." */
          if(g_common_packet_format_data_item.address_item.data.sequence_number == 0) {
            if(connection_object->eip_level_sequence_count_consuming == 0xFFFFFFFF) {
            ...
            if(g_common_packet_format_data_item.address_item.data.sequence_number > 
                connection_object->eip_level_sequence_count_consuming) {
              /* reset the watchdog timer */
              ConnectionObjectResetInactivityWatchdogTimerValue(connection_object);
