...
/** @brief Create Message Router Request structure with Key Format out of the received data.
 *
 * Parses the UCMM header consisting of: service, IOI size, IOI, data into a request structure
 * @param data pointer to the message data received
 * @param data_length number of bytes in the message
 ...
 * @return kEipStatusOk on success. otherwise kEipStatusError
 */
CipError CreateMessageRouterRequestKeyFromatStructure(
  const EipUint8 *data,
  EipInt16 data_length,
  CipMessageRouterRequest *message_router_request,
...
#if 1 /* Renesas EIP Sample : CT18.1 Error "The DUT does not support required Electronic Key Format 4." */
  if ( kCipErrorSuccess
       != ( status = CreateMessageRouterRequestKeyFromatStructure(
              data, data_length, &g_message_router_request, &extended_status) ) ) { /* error from create MR structure*/
    OPENER_TRACE_ERR(
...
CipError CreateMessageRouterRequestKeyFromatStructure(
  ...
  EipUint8 general_status = kCipErrorSuccess;

  message_router_request->service = *data;
  data++;
  data_length--;

  int number_of_decoded_bytes = DecodePaddedEPathKeyFromat(
    &(message_router_request->request_path), &data, &general_status, extended_status);
  if (general_status != kCipErrorSuccess) {
    return (CipError)general_status;
  }
  if (number_of_decoded_bytes < 0) {
    return kCipErrorPathSegmentError;
  }
