...
void R_Config_DSAD0_Create(void)
{
    ...
    AFE.OPCR.BIT.VREFEN = 1U;
    AFE.OPCR.BIT.DSAD0EN = 1U;

    /* Wait for 1ms for start-up time */
    for (w_count = 0U; w_count <= _1901_DSAD0_STARTUP_WAIT_COUNT; w_count++)
    {
        nop();
    }
    ...
    DSAD0.MR.LONG = _00000000_DSAD_CONTINUOUS_MODE | _00000004_DSAD_INTER_SYNC_ENABLE | _00000000_DSAD_CHANNEL0_USED | 
    ...
    DSAD0.CR0.LONG = _00100000_DSAD_PGA_ENABLE | _00070000_DSAD_PGA_GAIN_128;
    ...
    AFE.DS00ISR.WORD = _000B_AFE_POSIN_SIGNAL_AIN11 | _00A0_AFE_NEGIN_SIGNAL_AIN10 | _0400_AFE_REFVOL_REFOUT_AVSS0;
...
void R_Config_DSAD0_Start(void)
{
    ...
    DSAD0.MR.BIT.SYNCST = 1U;

    /* Wait for synchronization stable */
    for (w_count = 0U; w_count <= _001F_DSAD0_SYNCST_WAIT_COUNT; w_count++)
    {
        nop();
    }
